...
* added required field `bar`. Channels that uses older versions of the schema will have this field set to
`Injected`.

#### Version 1.0.1

* added optional field `foo`
...
* An required `bar` field to do something else
** Introduced in the schema version `2.0.0`
...
A channel is specified in the YAML language with a link:../core/src/main/resources/org/wildfly/channel/channel-schema-1.0.0.json[corresponding JSON schema] to validate its structure.
